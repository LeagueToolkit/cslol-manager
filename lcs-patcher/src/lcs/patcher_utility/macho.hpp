...
namespace LCS {
    struct MachO::Stream {
        ...
        inline T read_raw() {
            if (size_ < sizeof(T)) {
                throw "Failed to read raw: no more data!";
            }
        ...
        inline std::uint64_t read_uleb64() {
            ...
            for (int shift = 0; true; shift += 7) {
                if (shift + 7 > 63) {
                    throw "Failed to read uleb64: value too big!";
                }
        ...
        inline Stream copy(std::size_t offset = 0, std::size_t count = (std::size_t)-1) const {
            if (offset > size_) {
                throw "Failed to seek: out of range!";
            }
        ...
        inline void skip(std::size_t count) {
            if (count > size_) {
                throw "Failed to skip: out of range!";
            }
            data_ += count;
            size_ -= count;
        }
    ...
    inline void MachO::parse_file(char const* filename) {
        ...
        if (auto fd = ::open(filename, O_RDONLY); fd != -1) {
            if (struct stat s = {}; ::fstat(fd, &s) != -1) {
                result.resize(s.st_size);
                if (read(fd, result.data(), result.size()) <= 0) {
                    result.clear();
                }
            }
            close(fd);
        ...
        for(auto stack = std::vector<std::pair<std::string, std::size_t>> { { "", 0ull } }; !stack.empty();) {
            ...
            if (auto const info_len = stream.read_uleb64()) {
                if (auto const flags = stream.read_uleb64(); flags & 0x08) {
                    ...
                    exports[sym_name] = Export {
                        .stub_and_resolver = Export::StubAndResolver {
    ...
    inline void MachO::read_binding_lazy(Stream const& org_stream) {
        ...
        for (auto stream = org_stream; !stream.empty(); ) {
            ...
            case 0x90:
                ...
                break;
            default:
                throw "Unknown symbol binding opcode";
            }
        }
    }
